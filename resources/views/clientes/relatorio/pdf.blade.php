...
<body>
    ...
    <div class="secao">
        ...
        @if (! $relatorio['financeiro'])
            <div class="aviso">Este mês ainda não foi fechado no sistema — os dados financeiros não estão disponíveis para esta competência.</div>
        @else
            @php($f = $relatorio['financeiro'])
            <table class="resumo-grid">
                <tr>
                    <td><span class="resumo-label">Receita total</span><span class="resumo-valor">R$ {{ number_format($f->receitaTotal(), 2, ',', '.') }}</span></td>
                    <td><span class="resumo-label">Custo total</span><span class="resumo-valor">R$ {{ number_format($f->custoTotal(), 2, ',', '.') }}</span></td>
                    <td><span class="resumo-label">Lucro bruto</span><span class="resumo-valor {{ $f->lucro_bruto >= 0 ? 'verde' : 'vermelho' }}">R$ {{ number_format($f->lucro_bruto, 2, ',', '.') }}</span></td>
                    <td><span class="resumo-label">Margem</span><span class="resumo-valor">{{ $f->margem_percentual !== null ? number_format($f->margem_percentual, 1, ',', '.').'%' : '—' }}</span></td>
                </tr>
            </table>
            @if ($f->receita_excedente > 0)
                <p style="font-size: 10px; color: #b91c1c;">Inclui R$ {{ number_format($f->receita_excedente, 2, ',', '.') }} de excedente sobre o plano contratado.</p>
            @endif
        @endif
    </div>
