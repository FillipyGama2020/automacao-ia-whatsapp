    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Lista de bloqueio') }}
            </h2>
            <a href="{{ route('portal.contatos-bloqueados.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                {{ __('Novo contato') }}
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <p class="text-sm text-gray-500">
                Números cadastrados aqui nunca recebem resposta automática do seu assistente de IA —
                use pra parentes, amigos ou qualquer contato pessoal que use o mesmo número comercial.
            </p>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-4">
                ...
                <form method="POST" action="{{ route('portal.contatos-bloqueados.importar') }}" enctype="multipart/form-data" class="flex flex-wrap gap-3 items-center">
                    @csrf
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-4">
                <form method="GET" class="flex flex-wrap gap-3 items-end">
                    ...
                    @if (request()->filled('busca'))
                        <a href="{{ route('portal.contatos-bloqueados.index') }}" class="text-sm text-gray-500 underline">Limpar</a>
                    @endif
                </form>
            </div>
                        ...
                        @forelse ($contatos as $contato)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('portal.contatos-bloqueados.edit', $contato) }}" title="Editar"
                                       class="p-1.5 rounded-md text-indigo-600 hover:bg-indigo-50 hover:text-indigo-900 inline-flex">
                                        <x-icon name="edit" />
                                    </a>
                                    <form action="{{ route('portal.contatos-bloqueados.destroy', $contato) }}" method="POST" class="inline"
